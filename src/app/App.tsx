...
export default function App() {
  ...
  const [searchEngine, setSearchEngine] = useState(
    () => localStorage.getItem('navatation_search_engine') || 'google',
  )
  ...
  const handleAddHomeShortcuts = useCallback(
    (newShortcuts: any[]) => {
   ...
   * 处理网格捷径拖拽结束
   */
  const handleDragEndGrid = useCallback(
    (event: DragEndEvent) => {
  ...
  const handleDeleteHomeShortcut = useCallback(
    (index: number) => {
  ...
  const handleEditHomeShortcut = useCallback(
    (index: number) => {
  ...
  const handleSaveHomeShortcutEdit = useCallback(
  ...
  const activeDragShortcut = activeDragId
    ? displayShortcuts.find((s, idx) => (s.dragId || `shortcut-edit-${idx}`) === activeDragId)
    : null
    ...
    <ContextMenu>
      ...
      <ContextMenuContent className="w-56 z-[100] bg-widget-bg/95 backdrop-blur-xl border border-widget-border text-foreground shadow-2xl p-1.5 rounded-xl">
        <ContextMenuItem
          ...
          className="gap-3 cursor-pointer rounded-lg p-2.5 text-[15px] font-medium transition-colors"
        >
