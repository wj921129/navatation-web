...
import { Check, ChevronRight, Copy, Layers, Layout, RefreshCw, Sparkles, X } from 'lucide-react'
import { useEffect, useRef, useState } from 'react'
import { BaseModal } from '../ui/BaseModal'
import { Tooltip } from '../ui/Tooltip'
import { AiSearchFooter } from './AiSearchFooter'
import { AiSynthesisPanel } from './AiSynthesisPanel'
import { generateSynthesisAnswer, getKnowledgeAnswer } from './aiSearchLogic'
...
export function AiSearchOverlay({
  ...
  const [viewMode, setViewMode] = useState<'split' | 'focus'>(
    initialEngine === 'ai-aggregator' ? 'split' : 'focus',
  )
  const [activeEngine, setActiveEngine] = useState<string>(
    initialEngine === 'ai-aggregator' ? 'chatgpt' : initialEngine,
  )
        ...
        {viewMode === 'split' ? (
                ...
                >
                  ...
                  <div className="px-5 py-4 border-b border-white/5 flex items-center justify-between bg-white/2 backdrop-blur-md">
                    <div className="flex items-center gap-2.5">
                      ...
                      {engine.isStreaming && (
                        <span
                          className="w-1.5 h-3.5 bg-current animate-pulse rounded"
                        ...
                        {engine.streamedContent.split('\n').map((line, idx) => {
                          if (line.startsWith('###'))
                            return (
                              <h4
                                key={idx}
                                className={`text-sm font-extrabold ${engine.textGlow} mt-4 first:mt-0`}
                              >
                                {line.replace('###', '').trim()}
                              </h4>
                            )
                          if (line.startsWith('####'))
                            return (
                              <h5 key={idx} className="text-xs font-bold text-slate-200 mt-3">
                                {line.replace('####', '').trim()}
                              </h5>
                            )
                          if (line.startsWith('-'))
                          ...
                          if (line.startsWith('`') || line.includes('```')) {
                            ...
                            return (
                              <pre
                                key={idx}
                                className="bg-slate-900/80 border border-white/5 p-3 rounded-2xl font-mono text-xs text-slate-300 my-2 overflow-x-auto whitespace-pre"
                              >
                                {line.replace(/`/g, '')}
                              </pre>
                            )
          ...
          <div className="flex-1 flex max-w-4xl mx-auto w-full gap-6 items-stretch min-h-[400px]">
            ...
            <div className="w-52 bg-white/3 border border-white/5 rounded-3xl p-3 flex flex-col gap-2">
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-3 py-1.5">
                选择AI引擎
              </span>
              {Object.keys(engines).map((key) => {
                const engine = engines[key]
              ...
              <div className="flex-1 flex flex-col rounded-3xl border border-white/5 bg-slate-950/45 dark:bg-black/55 backdrop-blur-xl overflow-hidden shadow-2xl">
                ...
                <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between bg-white/2 backdrop-blur-md">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-slate-400 ml-3 font-mono">
                        LATENCY: {engines[activeEngine].speed}ms | TOKENS:{' '}
                        {engines[activeEngine].tokenCount}
                      </span>
                    </div>
                  </div>
                  ...
                  <Tooltip content="复制原文" side="top">
                    <button
                      onClick={() =>
                        handleCopy(engines[activeEngine].streamedContent, activeEngine)
                      }
                      ...
                      {engines[activeEngine].streamedContent.split('\n').map((line, idx) => {
                        if (line.startsWith('###'))
                          return (
                            <h4
                              key={idx}
                              className={`text-base font-extrabold ${engines[activeEngine].textGlow} mt-6 first:mt-0`}
                            >
                              {line.replace('###', '').trim()}
                            </h4>
                          )
                        if (line.startsWith('####'))
                          return (
                            <h5 key={idx} className="text-sm font-bold text-slate-100 mt-4">
                              {line.replace('####', '').trim()}
                            </h5>
                          )
                        if (line.startsWith('-'))
                          return (
                            <li key={idx} className="text-slate-300 ml-5 list-disc">
                              {line.replace('-', '').trim()}
                            </li>
                          )
                        if (line.startsWith('`') || line.includes('```')) {
                          ...
                          return (
                            <pre
                              key={idx}
                              className="bg-slate-900/80 border border-white/5 p-4 rounded-2xl font-mono text-xs text-slate-300 my-3 overflow-x-auto whitespace-pre"
                            >
                              {line.replace(/`/g, '')}
                            </pre>
                          )
