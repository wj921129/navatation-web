 ...
 * @date 2026-06-09
 */
import { Link, Upload, Loader2, RotateCw, Check, X } from 'lucide-react';
      ...
      >
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">输入网址，自动获取图标或手动设置</p>
          </div>
        </div>
        ...
        <div className="space-y-5">
          ...
          <div>
            <label className="block text-sm text-gray-600 dark:text-gray-400 mb-2">网站名称 *</label>
            <input
              type="text"
              value={customName}
            ...
            <div className="flex gap-2 items-center">
              <div className="flex-1 relative">
                <input
                  ...
                  readOnly={detectedIcons.length > 0 && detectedIcons.includes(customIconUrl) && !iconFromUpload}
                  onChange={(e) => {
                  ...
                  className={`w-full px-4 py-3 pr-10 bg-background border border-border rounded-xl outline-none transition-all h-[46px] ${
                    detectedIcons.length > 0 && detectedIcons.includes(customIconUrl) && !iconFromUpload
                  ...
                  title={(detectedIcons.length > 0 && detectedIcons.includes(customIconUrl) && !iconFromUpload) ? "搜索结果不可编辑，可双击复制" : "网址图标链接"}
                />
              ...
              <button
                ...
                disabled={!customUrl.trim() || faviconStatus === 'loading' || faviconStatus === 'uploading'}
                  ...
                  {detectedIcons.map((iconUrl, idx) => (
                    <button
                      ...
                      className={`w-12 h-12 flex-shrink-0 bg-card shadow-sm border rounded-xl flex items-center justify-center overflow-hidden transition-all cursor-pointer ${
                        customIconUrl === iconUrl && showImagePreview ? 'border-blue-500 ring-2 ring-blue-500/20' : 'border-border hover:border-gray-400 dark:hover:border-gray-500'
                      }`}
                    >
                      <img
                        src={iconUrl}
            ...
            {detectedIcons.length === 0 && showImagePreview && customIconUrl && (
              <div className="mt-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{customIconUrl}</p>
                  <button
                    onClick={() => {
                      setCustomIconUrl('');
                    }}
