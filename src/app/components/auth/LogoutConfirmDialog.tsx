...
export function LogoutConfirmDialog({ isOpen, onClose, onConfirm, username }: LogoutConfirmDialogProps) {
  ...
  return (
    <BaseModal
      ...
      animationType="scale"
      position="center"
      containerClassName="w-full max-w-sm glass-panel rounded-3xl shadow-2xl dark:shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] p-8 text-center"
    ...
    >
      ...
      {view === 'confirm' ? (
        ...
        <form onSubmit={handleChangePassword} className="text-left space-y-4">
          ...
          {error && (
            <div className="px-4 py-2.5 bg-red-50 dark:bg-red-950/45 border border-red-200/50 dark:border-red-900/30 rounded-xl text-xs text-red-600 dark:text-red-400 shadow-sm">
              {error}
            </div>
          )}

          {success && (
            <div className="px-4 py-2.5 bg-green-50 dark:bg-green-950/45 border border-green-200/50 dark:border-green-900/30 rounded-xl text-xs text-green-600 dark:text-green-400 shadow-sm">
              {success}
            </div>
          )}
