...
export function UnifiedDragItem({ 
  ...
  return (
    <div className={`flex flex-col items-center relative cursor-pointer ${wrapperClassName}`} style={{ width: `${iconSize + 32}px` }}>
      <div 
      ...
      >
        {(() => {
          if (shortcut.iconType === 'CUSTOM_URL' || shortcut.iconType === 'FAVICON' || shortcut.iconType === 'CUSTOM_UPLOAD') {
            return <img src={shortcut.iconValue} alt={shortcut.name} style={{ width: '50%', height: '50%', objectFit: 'contain' }} />;
          }
          if (shortcut.icon && typeof shortcut.icon !== 'string') {
             const IconComp = shortcut.icon;
             return <IconComp style={{ color: shortcut.color || shortcut.iconColor || '#333', width: `${innerSize}px`, height: `${innerSize}px` }} strokeWidth={2} />;
          }
