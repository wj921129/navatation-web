...
export function WeatherSettingsModal({
  ...
  const searchCity = useCallback(async (searchQuery: string) => {
    ...
    try {
      const res = await fetch(
        `https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(searchQuery)}&count=5&language=zh`,
      )
      if (res.ok) {
  ...
  return (
    <BaseModal isOpen={isOpen} onClose={onClose} animationType="scale" position="center">
      <div
        className="w-[400px] bg-widget-bg/95 backdrop-blur-2xl border border-widget-border rounded-3xl p-6 shadow-2xl flex flex-col gap-6"
      ...
      >
          ...
          {results.length > 0 && query && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-widget-bg border border-widget-border rounded-xl shadow-xl z-10 overflow-hidden max-h-[200px] overflow-y-auto">
              {results.map((r, i) => (
                <div
                  key={i}
                  className="px-4 py-3 hover:bg-input-bg cursor-pointer flex items-center justify-between transition-colors group"
                  onClick={() => handleAdd(r)}
                >
        ...
        <div className="flex flex-col gap-3">
          <h3 className="text-[13px] font-medium text-text-secondary uppercase tracking-wider">
            已选城市 ({locations.length})
          </h3>
          <div className="flex flex-col gap-2 max-h-[200px] overflow-y-auto pr-1">
            {locations.length === 0 ? (
              ...
              locations.map((loc, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between p-3 rounded-xl bg-input-bg/50 border border-widget-border hover:bg-input-bg transition-colors"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-blue-500/10 flex items-center justify-center">
                      <MapPin className="w-4 h-4 text-blue-500" />
                    </div>
                    <span className="text-[14px] font-medium text-text-primary truncate max-w-[220px]">
                      {loc.name.split(',')[0]}
                    </span>
                  </div>
                  <button
                    onClick={() => handleRemove(i)}
                    className="p-1.5 text-text-secondary hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-all"
                    aria-label="移除城市"
                  >
