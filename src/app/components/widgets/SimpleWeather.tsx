          ...
          if (ipRes.ok) {
            ...
            if (data && data.lat && data.lon && isMounted) {
              setResolvedLocations([
                { name: data.city || '当前位置', lat: data.lat, lon: data.lon },
              ])
              return
            }
          }
    ...
    >
      ...
      {hasMultiple && (
        <>
          <button
            onClick={handlePrev}
            className="absolute left-1 top-1/2 -translate-y-1/2 p-1 text-text-secondary hover:text-text-primary opacity-0 group-hover/weather:opacity-100 transition-opacity z-10 bg-widget-bg/50 backdrop-blur rounded-full"
          >
            <ChevronLeft className="w-4 h-4" />
          </button>
          <button
            onClick={handleNext}
            className="absolute right-1 top-1/2 -translate-y-1/2 p-1 text-text-secondary hover:text-text-primary opacity-0 group-hover/weather:opacity-100 transition-opacity z-10 bg-widget-bg/50 backdrop-blur rounded-full"
          >
      ...
      <div className="flex flex-col items-center gap-1 mt-1 transition-opacity duration-300 relative z-0">
        {currentWeather ? (
          ...
          <div className="w-8 h-8 animate-pulse bg-input-bg rounded-full"></div>
        )}
